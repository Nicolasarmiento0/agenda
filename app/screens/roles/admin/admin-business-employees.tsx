    ...
    <Modal transparent visible={visible} animationType="fade" onRequestClose={onClose}>
      <View style={[styles.modalOverlay, { backgroundColor: 'rgba(0,0,0,0.6)' }]}>
        <View style={[styles.modalContent, { backgroundColor: colors.background, borderColor: colors.border }]}>
          ...
          {!initialData && (
            <>
              <Text style={[styles.modalLabel, { color: colors.textSecondary }]}>Correo de Acceso (Obligatorio)</Text>
              <TextInput
                style={[styles.modalInput, { color: colors.textPrimary, borderColor: colors.border, backgroundColor: colors.surface }]}
                value={email}
              ...
              <Text style={[styles.modalLabel, { color: colors.textSecondary }]}>Contraseña Temporal (Obligatorio)</Text>
              <TextInput
                style={[styles.modalInput, { color: colors.textPrimary, borderColor: colors.border, backgroundColor: colors.surface }]}
                value={password}
...
export default function AdminBusinessEmployeesScreen() {
  ...
  const handleSaveEmp = useCallback(async (data: Partial<Employee> & { email?: string, password?: string }) => {
    ...
    if (editingEmp) {
      ...
      // Crear usuario en Supabase Auth usando cliente temporal
      ...
      if (newUserId) {
        // Garantizar la creación o actualización del perfil del trabajador con el rol 'worker'
