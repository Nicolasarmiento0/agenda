...
export default function CompanyHistoryScreen() {
    ...
    try {
      ...
      if (business?.category_id) {
        const { data: catData } = await supabase
          ...
          .single();
        if (catData) {
          setIsGym(catData.name.toUpperCase().includes('GIMNASIO') || catData.name.toUpperCase().includes('FITNESS'));
        }
      }

      // 1. Obtener trabajadores para el filtro
  ...
  }, [business?.id, selectedWorkerId, timeRange]);
      ...
      <View style={styles.filtersContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll}>
          ...
          <TouchableOpacity 
            ...
            style={[styles.filterChip, selectedWorkerId === 'all' && { backgroundColor: '#444', borderColor: '#444' }]}
          >
            <Text style={[styles.filterChipText, { color: selectedWorkerId === 'all' ? '#fff' : colors.textSecondary }]}>TODOS</Text>
          </TouchableOpacity>
          {workers.map(w => (
            <TouchableOpacity 
              key={w.id}
              onPress={() => setSelectedWorkerId(w.id)}
              style={[styles.filterChip, selectedWorkerId === w.id && { backgroundColor: '#444', borderColor: '#444' }]}
            >
              <Text style={[styles.filterChipText, { color: selectedWorkerId === w.id ? '#fff' : colors.textSecondary }]}>{w.name.toUpperCase()}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
