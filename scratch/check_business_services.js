...
async function checkBusinessServices() {
  const { data, error } = await supabase.from('business_services').select('*').limit(5);
  if (error) {
    console.error('Error fetching business_services:', error);
    return;
  }
  console.log('Business Services:', JSON.stringify(data, null, 2));
}
