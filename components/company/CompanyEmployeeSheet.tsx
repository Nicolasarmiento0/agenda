...
type Props = {
  ...
  colors: { textPrimary: string; textSecondary: string; surface: string; border: string };
};

export default function CompanyEmployeeSheet({ employee, visible, onClose, onAction, colors }: Props) {
      ...
      >
        ...
        <View style={styles.top}>
          <View style={[styles.avatar, { backgroundColor: employee.color }]}>
            <Text style={styles.avatarInitial}>{employee.initials}</Text>
          </View>
          <View style={{ flex: 1 }}>
        ...
        <View style={styles.actions}>
          {ACTIONS.map((a) => (
            <TouchableOpacity
              ...
              style={styles.action}
              activeOpacity={0.7}
              onPress={() => { onClose(); onAction(a.id, employee); }}
            >
